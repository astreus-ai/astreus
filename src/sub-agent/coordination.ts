...
export class SubAgentCoordinator {
    ...
    for (const taskResult of taskResults) {
      if (taskResult.status === 'fulfilled') {
        ...
        const errorMessage = taskResult.reason instanceof Error ? 
          taskResult.reason.message : String(taskResult.reason);
   ...
   */
  private enhanceTaskWithContext(task: SubAgentTask, previousResults: SubAgentResult[]): SubAgentTask {
    ...
    const dependencyResults = previousResults.filter(r => 
      task.dependencies!.includes(r.agentId) && r.success
    );
    ...
    const contextParts = dependencyResults.map(r => 
      `Result from ${r.agentName}: ${r.result}`
    );
